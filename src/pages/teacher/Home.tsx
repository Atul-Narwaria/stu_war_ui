...
export default function Home() {
  const [totalStudent, setTotalStudent] = useState<any>(null);
  ...
  useEffect(()=>{
    const apis = async()=>{
      let students:any = await teacherTotalStudents();
        if(students.status === "success"){
          setTotalStudent(students.message);
        }
      let totalbatchs = await teacherTotalbatch();
      if(totalbatchs.status === "success"){
        setTotalBatch(totalbatchs.message);
      }
      let todaybatches = await teacherbatch()
      if(todaybatches.status === "success"){
        setTodayBatch(todaybatches.message);
       
      }
      let batchList = await teacherbatchStatus();
      if(batchList.status === "success"){
        setBatchList(batchList.message);
      }
      let events = await TeacherEvents();
  ...
  const handleLiveClass = async(id:any)=>{
    ...
    console.log(meeting,password)
      navigate(`/teacher/zoom_class/${encryptUUID(meeting)}/${encryptUUID(password)}`)
  }
              ...
              <div className="cards p-2 grid grid-col-1 lg:grid-cols-3 gap-3">
                ...
                }
               {
                ...
                (
                  ...
                  <div className="flex justify-between align-middle gap-10 py-3    ">
                        <div >
                          <p className=" font-semibold text-md text-gray-500">Total Students</p>
                          <p className=" font-bold text-xl text-gray-900">
                            {totalStudent.toString().length > 1 ? totalStudent  : "0"+totalStudent}
                          </p>
                        </div>
                        <div className="mt-1">
                          <div className="bg-primary p-3 rounded-lg">
