...
export const getInstituteTeacherActiveAll = async () => {
  try {
    const get = await instance.get(`/institute/teacher/get/all/active`);
    return get.data;
  } catch (e: any) {
    return { code: e.response.status, message: e.message };
  }
...
export const getInstituteTeacherSearch = async (query: any, page?: number) => {
  try {
    ...
    const get = await instance.get(
      `/institute/teacher/get/search?page=${pg}&query=${query}`
    );
    return get.data;
  } catch (e: any) {
    return { code: e.response.status, message: e.message };
...
export const updateInstitueTeacherStatus = async (
  id: string,
  status: boolean
) => {
  try {
...
export const editInstitueTeacherStatus = async (
  id: string | any,
  firstname: string,
  lastname: string,
  email: string,
